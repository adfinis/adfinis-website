...
if ($response === false) {
  die("Fout bij ophalen van data");
}
$strapiFiles = json_decode($response, true);
if (!is_array($strapiFiles)) {
  die("Ongeldige JSON respons");
}

$extracted = [];
foreach ($strapiFiles as $item) {
  $id = $item['id'] ?? 'N/A';
...
//  echo "filename: $filename\n";
  ...
  $matches = array_filter($extracted, function($item) use ($filename) {
    return isset($item['name']) && str_starts_with($item['name'], $filename);
  });

  if (!empty($matches)) {
    $strapiFileInfo = array_values($matches)[0];
    ...
    foreach($item['ids'] as $id) {
      $exportItems[] = [
        ...
        'wp_url' => $url,
        'filename' => $filename,
        'strapi_cdn_url' => $strapiFileInfo['url'],
        'strapi_file_id' => $strapiFileInfo['id'],
        'strapi_file_documentid' => $strapiFileInfo['documentId'],
      ];
    }
//    echo $url . " => " . $matches[0]['url'] . "\n";
  }
}
var_dump($exportItems);
echo PHP_EOL;

$jsonString = json_encode($exportItems, JSON_PRETTY_PRINT);

// Wegschrijven naar bestand
file_put_contents('local-dev-mau-assets.json', $jsonString);

//var_dump(
//  $result,
//  count($totalIds),
//  count(array_unique($totalIds)),
//  count($totalUrls),
//  count(array_unique($totalUrls)),
//);
